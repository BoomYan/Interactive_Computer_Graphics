...
inline Quat interpolateCatmullRom(const Quat& c0, const Quat& c1, const Quat& c2, const Quat& c3, const double alpha) {

  Quat d =  power(cn(c2 * inv(c0)), 1.0/6.0) * c1;
  Quat e =  power(cn(c3 * inv(c1)), -1.0/6.0) * c2;

  Quat f = slerp(c1, d, alpha);
  Quat g = slerp(d, e, alpha);
  Quat h = slerp(e, c2, alpha);
